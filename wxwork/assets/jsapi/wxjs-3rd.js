...
var vsprintf = function(fmt, argv) {
  ...
  return sprintf.apply(null, argv);
    ...
    function _checkRegisterEvent() {
      ...
      var retMap = {};
      retMap[_MESSAGE_TYPE] = "checkRegister";
      retMap['params'] = {'registerd':json, 'wx':window.wx?true:false}, retMap[_SHA_KEY] = '';
      retMap['__callback_id']='';
      retMap['func']="checkRegister";
      var ret = JSON.stringify(retMap);
      _setResultValue('SCENE_FETCHQUEUE', ret);
      return ret;
    }
    ...
    function _getdescription() {
        ...
        var nodeList = document.getElementsByName("description");
      ...
      _on('menu:share:timeline',function(argv){
        ...
        if (typeof argv.title === 'string') {
          ...
          data = {
              ...
              "desc": _getdescription() ||  _session_data.init_url,
    ...
    // share with wechat
    _on('menu:share:wechat',function(argv){
      _log('share wechat');

      var data;
      if (typeof argv.title === 'string') {
        data = argv;
        _call('shareWechat',data);
      }else{
        data = {
            // "appid" : ""
            // "img_url": "",
     ...
     _on('onHistoryBack',function(argv) {
        _log('onHistoryBack');
        var data = {};
        _call('historyBack',data);
      });
    ...
    function _getallhref() {
        ...
        var list = [];
        list = document.getElementById("attachment").getElementsByTagName("a");
        for(var i = 0; i < list.length; i++) {
            var url = list[i].getAttribute("onclick");
             urls = urls + url;
             urls = urls + " ";
        }
        return urls;
     ...
     _on('sys:gethref',function(argv) {
      ...
      _on('sys:init',function(ses){
		...
		if(!window.WeixinJSBridge._hasPreInit){
		    console.log("weixin://preInjectJSBridge/ok2");
		    _session_data = ses;
		}else{
    ...
    if (window.WeixinJSBridge) {
      var a = window.WeixinJSBridge._hasInit;
      var b = window.WeixinJSBridge._hasPreInit;
      _WXJS.extend(window.WeixinJSBridge,__WeixinJSBridge);
      window.WeixinJSBridge._hasInit = a;
      window.WeixinJSBridge._hasPreInit = b;
    }else{
...
(function(window) {
    ...
    evalWXjsApi(function() {
        window.WeixinJSBridge.onMenuShareWechat = function(a) {
            window.WeixinJSBridge.on("menu:share:wechat", function() {
                a.trigger && a.trigger(a);
                WeixinJSBridge.invoke("shareWechat", {
                    title: a.title,
                    link: a.link,
                    img_url: a.imgUrl,
                    desc: a.desc,
