...
console.log(["rbsw", _nRestoreBodyScrollWidth, "scale", _nCurScale, "pw", _nPageWidth, "si",
	_bIsShowImg, "ad", _bIsAdMail, "delay", _bIsDelayScale, "_bIsFixTable", _bIsFixTable].toString());
...
function _fixTableScaleReady(_aoDomArea) {
  ...
  var _cRect = _oDomTable.getBoundingClientRect();
  console.log("_fixTableScaleReady " + _cRect.right + "," + document.body.scrollWidth);
  if(_cRect.right == document.body.scrollWidth){
    //same width don't scale
    return;
  }
  _oDomTable.style.minWidth="";
  _oDomTable.style.maxWidth="";
  ...
  _aoDomArea.setAttribute("nOriWidth", _nScrollWidth = _oDomTable.scrollWidth);
    console.log("_canRemoveTablesWidth " + _canRemoveTablesWidth(_aoDomArea)+ " nOriWidth "+ _nScrollWidth);
  //记录原宽度，方便refix的时候使用
...
function _startLoadImages(_aoImgs, _asImgsInfo) {
  //var _oRealImgsResult = _asImgsInfo.split("#;#");
  //由于getImageCachePath实现原因，所以要从1开始
  for (var i = 0, _nLen = _aoImgs.length, _sSrc, _nWidth, _nHeight; i < _nLen; i++) {
      _sSrc = _aoImgs[i].src;
      ...
      if ((_nWidth > 50 && _nHeight > 50) || (_nWidth == 0 && _nHeight == 0)) {
       ...
       _aoImgs[i].src = "file:///android_asset/spacer.png?r=" + i;
...
//图片加载完成的处理逻辑
function _imgOnLoad() {
  console.log("imgonload ..."+_imgOnLoad._nTimer+" p "+this.parentNode);
  if (!this._nTimer) {
    this._nTimer = setTimeout(function(aNode) {
...
function _resetContentHeight() {
  ...
  var _nCurFillDivTop = Math.max(dom("#mailcontent").getBoundingClientRect().bottom, dom("#mailcontent").scrollHeight);
  _nCurFillDivTop = Math.max(_nCurFillDivTop, document.body.getBoundingClientRect().bottom);
  //warning!!!
  ...
  if (_resetContentHeight._nOldFillDivTop != _nCurFillDivTop || _resetContentHeight._nOldWebViewScale != _nWebViewScale) {
    ...
    _resetContentHeight._nOldFillDivTop = _nCurFillDivTop;
    window.location="mailcall://height/"+_nCurFillDivTop;
...
var _nCurTitleBarHeight = 0,//App.getTitleBarHeight(),
...
//公开给java层调用回传整个页面的html内容
window.getWebviewHtml = function() {
    //App.getWebviewHtml(document.documentElement.outerHTML);
}

window.resetContentHeight2 = function(){
    _resetContentHeight._nOldFillDivTop = 0;
	_resetContentHeight();
}
