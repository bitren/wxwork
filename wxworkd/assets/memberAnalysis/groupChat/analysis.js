    ...
    genDebugData: function (num) {
        var debugInfo = {
            ...
            'generalInfo': {
                ...
                // applyContactNum: 10,
                // customerIncr: 20,
                ChatRoomsAllCount: 100,//群聊总数
                NewAddChatRoomsCount: 12, // 新增群聊数
                ReceivedMsgChatRoomsCount: 11, // 有过消息的群聊数
                RoomMembersAllCount: 23, // 群成员总数
                NewAddRoomMembersCount: 4, // 新增群成员数
                SentMsgRoomMembersCount: 13, // 发过消息的群成员数
                ChatRoomMsgAllCount: 67, // 群聊消息总数
            },
            'filterInfo': {
                'type': 'ReceivedMsgChatRoomsCount',
                'date': 'month',
        ...
        var configOption = {
            ...
            tooltip: {
                positioner: function (x, y, point) {
                    var x = point.plotX;
                    if (point.plotX + theChart.tooltip.label.width + tooltipWidth > window.innerWidth) {
                        x = window.innerWidth - theChart.tooltip.label.width - tooltipWidth - 14;
                    }
...
var Analysis = {
    ...
    bindEvent: function () {
        ...
        $('.js-party,.js-party-wrap').off(self.TAP);
        $('.js-party,.js-party-wrap').on(self.TAP, function () {
            self.onPartyPicker();
        });
        ...
        if (!self.isNotFirst) {
            ...
            setTimeout(function () {
                self.autoClick = true;
                $('.js_secTable_tabGroupChat .js-filter-type').first().trigger(self.TOUCHSTART);
            }, 0)
        }
